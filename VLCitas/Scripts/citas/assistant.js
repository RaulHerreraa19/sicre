clControl = {
    ...
    Initialize: function () {
        ...
        $("body").on("change", "#doctor_uId", clControl.OnChangeDoctor);
        ...
        $("body").on("click", ".show-appointment-details", clControl.ShowAppoinmentDetails);
        $("body").on("click", ".show-appointment-details-doctor", clControl.ShowAppoinmentDetailsDoctor);
    ...
    AddCita: function () {
        ...
        $.ajax({
            ...
            data: {
                ...
                doctor_uId: $("#appointment-doctor_uId").val(),
    ...
    GetPacientesByDoctor: function (doctor_uId) {
    ...
    ShowCalendar: function () {
        ...
        clControl.citas_calendar = $('#calendar').fullCalendar({
            ...
            eventClick: function (calEvent, jsEvent, view) {
                $("#DetalleCita").data("appointment-id", calEvent.id);
                clControl.ShowAppoinmentDetails();
                return false;
            },
        ...
        $(".add-appointment-steps").steps({
            ...
            onStepChanging: function (event, currentIndex, newIndex) {
                ...
                if (newIndex>currentIndex) {
                    switch (currentIndex) {
                        case 0: //doctor
                            if (isNullOrEmpty($("#doctor_uId").val())) {
                                $("#doctor-warning-module").css("display", "block");
                                moveOn = false;
                            } else {
                                $("#dr-name").val($("#doctor_uId").text().trim());
                                clControl.GetPacientesByDoctor($("#doctor_uId").val());
                            }
                            break;
                        case 1: //calendar
    ...
    ShowFormToAddAnAppoinment: function () {
        ...
        clControl.ShowDoctorDataOnModal($("#doctor_uId").val());
        clControl.patientForm = $("#frm-patient").validate();
        $('#AddCita').modal('show');
        if (clControl.doctors.items.length == 1)
            $(".add-appointment-steps").steps("next");

    },
    OnChangeDoctor: function () {
        clControl.ShowDoctorDataOnModal($(this).val());
    },
    ShowDoctorDataOnModal: function (doctor_uId) {
        $("#doctor-warning-module").css("display", "none");
        $("#tbody-dr-schedule").html("");
        $("#appointment-doctor_uId").val(doctor_uId);
        var data = clControl.GetDoctorScheduleTable(doctor_uId);
        $("#dr-speciality").val(data.doctor.value3);
        $("#tbody-dr-schedule").html(data.html);
        clControl.DocumentResize();
    },
    GetDoctorScheduleTable: function (doctor_uId) {
        var doctor = $.grep(clControl.doctors.items, function (n, i) {
            return n.id == doctor_uId;
        });
        doctor = doctor[0];
        var html = "";
        $.each(doctor.data, function (i, item) {
            var days = [];
            if (item.lunes == true) days.push(js_language[user_lang].Monday);
            if (item.martes == true) days.push(js_language[user_lang].Tuesday);
            if (item.miercoles == true) days.push(js_language[user_lang].Wednesday);
            if (item.jueves == true) days.push(js_language[user_lang].Thursday);
            if (item.viernes == true) days.push(js_language[user_lang].Friday);
            if (item.sabado == true) days.push(js_language[user_lang].Saturday);
            if (item.domingo == true) days.push(js_language[user_lang].Sunday);
            var start_hours = item.start_hour.Hours < 10 ? "0" + item.start_hour.Hours : item.start_hour.Hours;
            var start_minutes = item.start_hour.Minutes < 10 ? "0" + item.start_hour.Minutes : item.start_hour.Minutes;
            var end_hour = item.end_hour.Hours < 10 ? "0" + item.end_hour.Hours : item.end_hour.Hours;
            var end_minutes = item.end_hour.Minutes < 10 ? "0" + item.end_hour.Minutes : item.end_hour.Minutes;
            html += '<tr>' +
                '<td class="">' + days.join() + '</td >' +
                '<td class="">' + start_hours + ":" + start_minutes + '</td >' +
                '<td class="">' + end_hour + ":" + end_minutes + '</td >' +
                '</tr>';
        });
        return {
            doctor: doctor,
    ...
    ShowAppoinmentDetailsDoctor: function () {
        $("#DetalleCita").data("appointment-id", $(this).data("appointment-id"));
        $("#DoctorDetails").modal('hide');
        clControl.ShowAppoinmentDetails();
    },
    ShowAppoinmentDetails: function () {
        var id = $("#DetalleCita").data("appointment-id");
        if (isNullOrEmpty(id)) 
            id = $(this).data("appointment-id");

        $("#btn-confirm-appointment").css("display", "none");
        $("#btn-delete-appointment").css("display", "none");
        ShowLoading(js_language[user_lang].Processing);
        $.ajax({
            type: "Post",
            datatype: "html",
            url: "/Citas/DetailsCita",
            data: {
                uId: id
            },
            cache: false,
            success: function (Result) {
                ...
                if (Result["TypeOfResponse"] == 0) {
                    var cita = Result.Data;
                    $("#details-dr-name").val(cita.doctor.first_name + ' ' + cita.doctor.last_name);
                    $("#details-appointment-consultory").val(cita.consultory.name);
                    $("#details-patient-name").val(cita.patient.nombre);
                    $("#details-appointment-date").val(moment(cita.cita_date).format('LLLL'));
                    $("#details-appointment-phone").val(cita.telefono);
                    $("#details-appointment-status").val(js_language[user_lang].AppointmentStatus[cita.status_id]);

                    if (isNullOrEmpty(cita.precio)) 
                        $("#details-appointment-cost").val("$0.00");
                    else 
                        $("#details-appointment-cost").val("$" + cita.precio);

                    if (isNullOrEmpty(cita.observaciones_asistente)) 
                        $("#details-appointment-observations-to-assistant").val(cita.observaciones_asistente);
                    else 
                        $("#details-appointment-observations-to-assistant").val(js_language[user_lang].NoComments);
                    if (isNullOrEmpty(cita.observacion_doctor))
                        $("#details-appointment-observations").val(cita.observaciones_asistente);
                    else
                        $("#details-appointment-observations").val(js_language[user_lang].NoComments);
                    if (cita.status_id == 3)
                        $("#btn-delete-appointment").css("display", "none");
                    else {
                        $("#btn-delete-appointment").css("display", "block");
                        $("#btn-delete-appointment").attr("data-uId", cita.uId);
                    }
                    if (cita.status_id == 1) {
                        $("#btn-confirm-appointment").css("display", "block");
                        $("#btn-confirm-appointment").attr("data-uId", cita.uId);
                    }
    ...
    ShowDoctorDetails: function () {
        ...
        $.each(citas, function (i, cita) {
            ...
            html += '<tr>' +
                ...
                '<td class=""><button data-appointment-id="' + cita.id + '" type="button" class="show-appointment-details-doctor btn '+btn+'"><i class="ft-eye"></i></button></td >' +
                '</tr>';
        });
        $("#tbody-dr-appointments").html(html);
